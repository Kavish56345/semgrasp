...
class PerceptionNode(Node):
    ...
    def _process(self):
        ...
        # ── Confidence filter ─────────────────────────────────────────
        if logits[0] < self._box_thresh:
            self.get_logger().warn(
                f"Low confidence: {logits[0]:.3f}", throttle_duration_sec=5.0
            )
            return
        ...
        # ── Convert box: normalized cxcywh → xyxy pixels (resized) ────
        box = boxes[0]
        cx, cy, bw, bh = box
        x1_r = (cx - bw / 2) * 640
        y1_r = (cy - bh / 2) * 480
        x2_r = (cx + bw / 2) * 640
        y2_r = (cy + bh / 2) * 480
        box_xyxy_resized = np.array([x1_r, y1_r, x2_r, y2_r])
        ...
        self.get_logger().info(
            f'[{text}] conf={logits[0]:.2f} pts={len(points_3d)} '
